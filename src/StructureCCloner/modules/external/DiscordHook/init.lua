...
local function send(url, data, headers)
  local request, message = http.post(url.."?wait=true", data, headers)
  if not request then
    return false
  end
  return true, textutils.unserializeJSON(request.readAll()).id
end

local function update(url, data, headers,messageid)
  local request, message = http.request({url = url.."/messages/"..messageid, body = data, headers = headers,method = "PATCH"})
  if not request then
    return false
  end
  return true
...
function DiscordHook.createWebhook(url)
  ...
  if not success then
    ...
    function _.send(message, username, avatar)
      ...
      local success, messageid = send(_.url, data, { ["Content-Type"] = "application/x-www-form-urlencoded", ["Source"] = "Minecraft/ComputerCraft/DiscordHook" })
      if success then _.sentMessages = _.sentMessages + 1 end
      return success,messageid
    end
    function _.update(message, username, avatar,messageid)
      expect("update", message, 1, "string")
      local data = "content=" .. textutils.urlEncode(message)
      if username then
        expect("update", username, 2, "string")
        data = data .. "&username=" .. textutils.urlEncode(username)
      end
      if avatar then
        expect("update", avatar, 3, "string")
        data = data .. "&avatar_url=" .. textutils.urlEncode(avatar)
      end

      local success = update(_.url, data, { ["Content-Type"] = "application/x-www-form-urlencoded", ["Source"] = "Minecraft/ComputerCraft/DiscordHook" },messageid)
      if success then _.sentMessages = _.sentMessages + 1 end
      return success
    end
